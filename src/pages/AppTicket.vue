...
export default {
  ...
  async sendFile() {
    ...
    try {
      ...
      this.messages.push({
        id: Date.now(),
        sender: 'user',
        text: '', // Пустое текстовое сообщение, так как отправлен файл
        type: 'file',
  ...
  async sendMessage() {
    // Проверка, что текстовое сообщение введено
    if (!this.newMessage) {
  ...
  >
    ...
    >
      ...
      >
        ...
        <img :src="message.url" alt="Uploaded File" v-if="message.name.endsWith('.jpg') || message.name.endsWith('.jpeg') || message.name.endsWith('.png') || message.name.endsWith('.gif')" style="max-width: 100%; height: auto;" />
      </div>
      <div class="avatar">
        <img
